...
@section('id-table')
    id="mt_ketluan_thanhtrakiemtra_table_dom"
@endsection
...
@section('thead-th')
    ...
    <th class="text-bold">Ngày kết luận</th>
    <th width="25%">
        {{-- Buttons --}}
    </th>
@endsection
...
@section('tbody-td')
    @if (count($result) > 0)
        @foreach($result as $col)
            <tr>
                <td>
                    <a href='mt-ketluan-thanhtrakiemtra-detail?id={{$col['id']}}' target='_blank'>
                        <i class='fa fa-link'></i>
                        {{$col['soquyetdinh']}}
                    </a>
                </td>
                <td>{{$col['name_mt_doanthanhtra_kiemtra']}}</td>
                <td>{{$col['name_donvi_quanly']}}</td>
                <td>{{$col['ngay_ketluan_date']}}</td>
                <td>
                    {{-- Buttons --}}
...
@section('custom-js')
    <script src="{{'apps/baocao-tonghop/mt-ketluan-thanhtrakiemtra/config.js'}}"></script>
@endsection

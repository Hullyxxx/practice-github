...
@section('advance-search-form')
    ...
    <div class="row">
        <div class="col-md-6 form-group mb-3">
            <label for="loaithuphi_id">Loại thu phí</label>
            <select id="loaithuphi_id" class="form-control">
            </select>
        </div>
        <div class="col-md-6 form-group mb-3">
            <label for="mucthuphi_id">Mức thu phí</label>
            <select id="mucthuphi_id" class="form-control">
            </select>
        </div>
    </div>
...
@section('thead-th')
    ...
    <th class="text-bold">Tổng tiền nộp</th>
    <th class="text-bold">Phương án nộp</th>
    <th width="25%">
        {{-- Buttons --}}
    </th>
@endsection
...
@section('tbody-td')
    @if (count($result) > 0)
        @foreach($result as $col)
            <tr>
                <td>
                    <a href='mt-phi-bvmt-detail?id={{$col['id']}}' target='_blank'>
                        <i class='fa fa-link'></i>
                        {{$col['nam_thuchien']}}
                    </a>
                </td>
                <td>{{$col['tong_tien_nop']}}</td>
                <td>{{$col['phuongan_nop']}}</td>
                <td>
                    {{-- Buttons --}}
    ...
    <div class="col-md-12">
        ...
        <div id="foreign_key">
            <ul class="nav nav-tabs" id="listsTab" role="tablist">
                ...
                <li class="nav-item ">
                    <a class="nav-link " data-toggle="tab" href="#list_mt_chitiet_phi_bvmt" role="tab" aria-controls="info_fk_2" aria-selected="false">
                        <i class="nav-icon i-Library mr-1"></i>
        ...
        <div id="related-lists" class="tab-content">
            ...
            <div class="tab-pane fade show " id="list_mt_chitiet_phi_bvmt" role="tabpanel" aria-labelledby="main-info-tab">
                {{-- DOM data fk_2 --}}
                <table class="display table-middle-center table table-striped table-bordered" style="width:100%">
                    <tbody id="list_mt_chitiet_phi_bvmt_tbody">
                    </tbody>
                </table>
            </div>
        </div>
...
@section('custom-js')
    <script src="{{'apps/baocao-tonghop/mt-phi-bvmt/config.js'}}"></script>
@endsection

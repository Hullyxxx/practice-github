...
@section('id-table')
    id="mt_baocao_khoiluong_phatsinh_chatthai_table_dom"
@endsection

@section('thead-th')
    <th class="text-bold">Số báo cáo</th>
    <th class="text-bold">Cơ sở xử lý chất thải</th>
    <th class="text-bold">Ngày báo cáo</th>
    <th class="text-bold">Tổng khối lượng phát sinh</th>
    <th width="25%">
        {{-- Buttons --}}
    </th>
@endsection
...
@section('tbody-td')
    @if (count($result) > 0)
        @foreach($result as $col)
            <tr>
                <td>
                    <a href='mt-baocao-khoiluong-phatsinh-chatthai-detail?id={{$col['id']}}' target='_blank'>
                        <i class='fa fa-link'></i>
                        {{$col['so_baocao']}}
                    </a>
                </td>
                <td>{{$col['name_coso_xuly_chatthai']}}</td>
                <td>{{$col['ngay_baocao_date']}}</td>
                <td>{{$col['tong_khoiluong_phatsinh']}}</td>
                <td>
                    {{-- Buttons --}}
    ...
    <div class="col-md-12">
        ...
        <div id="foreign_key">
            <ul class="nav nav-tabs" id="listsTab" role="tablist">
                ...
                <li class="nav-item">
                    <a class="nav-link" data-toggle="tab" href="#list_mt_chitiet_baocao_khoiluong" role="tab" aria-controls="info_fk_1" aria-selected="false">
                        <i class="nav-icon i-Library mr-1"></i>
        ...
        <div id="related-lists" class="tab-content">
            ...
            <div class="tab-pane fade" id="list_mt_chitiet_baocao_khoiluong" role="tabpanel" aria-labelledby="main-info-tab">
                {{-- DOM data fk_1 --}}
                <table class="display table-middle-center table table-striped table-bordered" style="width:100%">
                    <tbody id="list_mt_chitiet_baocao_khoiluong_tbody">
                    </tbody>
                </table>
            </div>
        </div>
...
@section('custom-js')
    <script src="{{'apps/baocao-tonghop/mt-baocao-khoiluong-phatsinh-chatthai/config.js'}}"></script>
@endsection

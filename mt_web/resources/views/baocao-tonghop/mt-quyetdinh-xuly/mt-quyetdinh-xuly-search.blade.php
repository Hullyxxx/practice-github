...
@section('tbody-td')
    @if (count($result) > 0)
        @foreach($result as $col)
            <tr>
                <td>
                    <a href='mt-quyetdinh-xuly-detail?id={{$col['id']}}' target='_blank'>
                        <i class='fa fa-link'></i>
                        {{$col['soquyetdinh']}}
                    </a>
                </td>
                <td>{{$col['name_donvi_quanly']}}</td>
                <td>{{$col['name_chucvu']}}</td>
                <td>{{$col['nguoi_ky']}}</td>
                <td>
                    {{-- Buttons --}}
...
@section('custom-js')
    <script src="{{'apps/baocao-tonghop/mt-quyetdinh-xuly/config.js'}}"></script>
@endsection

...
@section('advance-search-form')
    ...
    <div class="row">
        ...
        <div class="col-md-6 form-group mb-3">
            <label for="mt_coso_onmt_id">Cơ sở ô nhiễm môi trường</label>
            <select id="mt_coso_onmt_id" class="form-control">
            </select>
        </div>
        <div class="col-md-6 form-group mb-3">
        ...
        <div class="col-md-6 form-group mb-3">
            <label for="loaididoi_id">Loại di dời</label>
            <select id="loaididoi_id" class="form-control">
            </select>
        </div>
    </div>
...
@section('id-table')
    id="mt_quyetdinh_didoi_table_dom"
@endsection

@section('thead-th')
    <th class="text-bold">Số quyết định</th>
    <th class="text-bold">Cơ sở ô nhiễm môi trường</th>
    <th class="text-bold">Đơn vị cấp phép</th>
    <th class="text-bold">Loại di dời</th>
    <th width="25%">
        {{-- Buttons --}}
    </th>
@endsection
...
@section('tbody-td')
    @if (count($result) > 0)
        @foreach($result as $col)
            <tr>
                <td>
                    <a href='mt-quyetdinh-didoi-detail?id={{$col['id']}}' target='_blank'>
                        <i class='fa fa-link'></i>
                        {{$col['soquyetdinh']}}
                    </a>
                </td>
                <td>{{$col['name_mt_coso_onmt']}}</td>
                <td>{{$col['name_donvi_capphep']}}</td>
                <td>{{$col['name_loaididoi']}}</td>
                <td>
                    {{-- Buttons --}}
    ...
    <div class="col-md-12">
        ...
        <div id="foreign_key">
            <ul class="nav nav-tabs" id="listsTab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" data-toggle="tab" href="#list_mt_coso_onmt" role="tab" aria-controls="info_fk_1" aria-selected="false">
                        <i class="nav-icon i-Library mr-1"></i>
                        Cơ sở ô nhiễm moi trường
                    </a>
                </li>
            </ul>
        </div>
        <div id="related-lists" class="tab-content">
            <div class="tab-pane fade show active" id="list_mt_coso_onmt" role="tabpanel" aria-labelledby="main-info-tab">
                {{-- DOM data fk_1 --}}
                <table class="display table-middle-center table table-striped table-bordered" style="width:100%">
                    <tbody id="list_mt_coso_onmt_tbody">
                    </tbody>
                </table>
            </div>
        </div>
...
@section('custom-js')
    <script src="{{'apps/baocao-tonghop/mt-quyetdinh-didoi/config.js'}}"></script>
@endsection
